...
function NavbarButton(props: { text: string, url: string }) {
    return <Button variant={"text"} color={"inherit"} onClick={() => Router.push(props.url)}>{props.text}</Button>;
}
...
export default function Navigation() {
    ...
    return <>
        <AppBar position={"static"} sx={{display: "flex", flexFlow: "row"}}>
            ...
            <Box sx={{flexGrow: 1}}>
                <Toolbar>
                    <NavbarButton text={"Dashboard"} url={"/dashboard"}/>
                    <NavbarButton text={"Communities"} url={"/communities"}/>
                    <NavbarButton text={"My Interests"} url={"/interests"}/>
                </Toolbar>
            </Box>
            <Box sx={{flexGrow: 0}}>
                <Menu
