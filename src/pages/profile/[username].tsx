...
export default function Profile() {
	...
	if (userLoading || profileLoading || interestsLoading) {
		// If content is loading
		if (splitSecond) {
			// But we just opened the page
			return <Navigation />;
		} else {
			// If it's been a short while
			return (
				<>
					<Navigation />
		...
		if (user && username === getTechUsername(user)) {
			return (
				<>
					<Navigation />
					<Container sx={{ margin: '1em 0' }}>
		...
		if (profile.year) {
			yearCaption = (
				<>
					{profile.year}
					<sup>{ordinal(profile.year)}</sup> year
				</>
			);
		}
			...
			<>
				...
				<Container sx={{ margin: '1em 0' }}>
					...
					>
						...
						>
							{yearCaption}
							{profile.year && profile.major ? <>&nbsp;&mdash;&nbsp;</> : null}
							{profile.major}
						</Typography>
						<Typography
							variant={'body1'}
