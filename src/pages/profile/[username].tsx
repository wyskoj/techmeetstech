...
export default function Profile() {
	...
	if (userLoading || profileLoading || interestsLoading) { // If content is loading
		...
		if (username === getTechUsername(user!)) {
			return <>
				<Navigation/>
				<Container sx={{margin: "1em 0"}}>
					<Stack sx={{display: "flex", flexDirection: "column", alignItems: "center"}} spacing={2}>
						<Typography><b>You don&apos;t have a profile setup yet!</b> Let&apos;s get started.</Typography>
						<EditProfileButton/>
					</Stack>
				</Container>
			</>
					...
					<Grid item xs={4}>
						<Card>
							<CardContent>
								...
								<ul>{interests?.map(it => <li key={it}>{it}</li>)}</ul>
							</CardContent>
						</Card>
					</Grid>
					<Grid item xs={4}>
